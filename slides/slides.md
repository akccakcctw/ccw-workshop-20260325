...
### 練習 B：用 Mixpanel 數據做分析

1. 從 Mixpanel 匯出你想分析的**報表 CSV**（用戶名單、事件數據、漏斗等）
2. 拖曳到 Claude Desktop
3. 試試這樣問：

<div class="p-3 bg-green-50 rounded mt-2 text-xs">

「這是我們 Mixpanel 匯出的用戶事件數據，幫我分析哪些行為跟轉換率最相關，找出產品增長的關鍵因素」

</div>

<div class="p-3 bg-blue-50 rounded mt-2 text-xs">

「根據這份漏斗數據，幫我找出流失最嚴重的步驟，分析可能原因，並產出一份給主管看的增長分析報告」

</div>

4. 追問深入：「把高價值用戶跟低價值用戶的行為差異列出來」
...
右邊是用 Mixpanel 數據做分析——從 Mixpanel 匯出 CSV，
丟給 Claude 讓它幫你找出增長關鍵因素、產出分析報告。
兩個挑一個做就好，做完再試另一個。
卡住了隨時舉手問我們。」
-->
